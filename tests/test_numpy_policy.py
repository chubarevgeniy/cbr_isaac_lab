...
def _write_toy_policy(path) -> None:
    ...
    weights = [
        rng.normal(size=(8, 19)).astype(np.float32),
    ...
    np.savez(
        ...
        weight_1=weights[1],
        bias_1=biases[1],
        obs_mean=np.linspace(-1.0, 1.0, 19, dtype=np.float32),
        obs_variance=np.linspace(0.5, 1.5, 19, dtype=np.float32),
...
def test_numpy_policy_matches_manual_mlp(tmp_path) -> None:
    ...
    observations = np.random.default_rng(8).normal(size=(5, 19)).astype(np.float32)
...
def test_observation_and_action_contract_uses_canonical_signs(tmp_path) -> None:
    ...
    observation = policy.build_observation(raw_positions, raw_velocities, [1.0, -0.4], [0.1] * 4)
    ...
    np.testing.assert_allclose(observation[15:], [0.1] * 4)
...
def test_sidecar_metadata_is_loaded(tmp_path) -> None:
    ...
    assert build_observation(
        np.zeros(7), np.zeros(7), np.zeros(2), np.zeros(4), 2.0
    ).shape == (19,)
