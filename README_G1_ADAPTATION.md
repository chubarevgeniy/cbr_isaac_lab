...
R_walk = +1.0 * exp(-((body_vel - target_speed) / 0.5)^2)
          ...
          -2.0 * sum((target_t - 2*target_{t-1} + target_{t-2})^2)
...
- `action_rate` штрафует вторую разность target: равномерный прирост не
  штрафуется, а разворот команды получает квадратичный penalty;
...
компоненты текущего `last_action` и четыре компоненты `second_last_action`.
Это даёт policy историю, необходимую для вычисления разницы двух приростов.
Reference
...
joint velocity, action-difference, limits и alive. Отличаются только pose targets:
...
| action difference | `-2.0` | `-2.0 * sum((target_t - 2*target_{t-1} + target_{t-2})²)` |
...
| `action_rate` | `-2.0` | изменение прироста target между шагами | `-2.0 * sum((target_t-2*target_{t-1}+target_{t-2})²)` | Эксперимент |
...
- `RewardComponents/positive_total`, `RewardComponents/negative_total` и
  отдельные `RewardComponents/<term>`; в частности, вклад
  `RewardComponents/action_second_difference`;
