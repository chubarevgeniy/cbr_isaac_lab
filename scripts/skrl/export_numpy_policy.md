...
`is_sitting` is `1` for sitting and `0` for standing/walking. Values
`obs[15:19]` are the exact previous policy action and `obs[19:23]` are the
action from two policy steps ago; neither is a target angle. Reset both
histories to zeros when the ROS node starts or the episode resets.
